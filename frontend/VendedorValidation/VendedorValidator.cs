...
namespace VendedorValidation
{
    public class VendedorValidator : AbstractValidator<VendedorVali>
    {
        public VendedorValidator()
        {
            ...
            // Validación para Ingresos de Ventas: Si está vacío muestra "Los ingresos de ventas son obligatorios"
            RuleFor(v => v.IngresosVentas)
                .NotEmpty().WithMessage("Los ingresos de ventas son obligatorios.");

            // Validación para Ingresos de Ventas: Si no está vacío valida formato y valor mayor que 0
            RuleFor(v => v.IngresosVentas)
                .Matches(@"^\d+(\.\d{1,2})?$").WithMessage("Los ingresos de ventas deben ser un número válido.")
                .Must(s => double.TryParse(s, out double result) && result > 0)
                .WithMessage("Los ingresos de ventas deben ser mayores que 0.")
                .When(v => !string.IsNullOrEmpty(v.IngresosVentas)); // Solo valida si los ingresos no están vacíos

            // Validación para Porcentaje de Comisión: Si está vacío muestra "El porcentaje de comisión es obligatorio"
            RuleFor(v => v.PorcentajeComision)
                .NotEmpty().WithMessage("El porcentaje de comisión es obligatorio.");

            // Validación para Porcentaje de Comisión: Si no está vacío valida formato y rango
            RuleFor(v => v.PorcentajeComision)
                .Matches(@"^\d+(\.\d{1,2})?$").WithMessage("El porcentaje de comisión debe ser un número válido.")
                .Must(s => double.TryParse(s, out double result) && result >= 0 && result <= 15)
                .WithMessage("El porcentaje de comisión debe estar entre 0 y 15.")
                .When(v => !string.IsNullOrEmpty(v.PorcentajeComision)); // Solo valida si el porcentaje de comisión no está vacío
        }
