...
//        testPersonaBOEliminar(listaId);
    ...
    private static void testPersonaBOExistePersona() {
        System.out.println("\ntestPersonaBOExistePersona");
        Boolean existe = documentoBO.existeDocumento(1,Estado.COTIZACION,Tipo.COMPRA,1,1,1);
        System.out.println("Existe documento: " + existe);
        existe = documentoBO.existeDocumento(1,Estado.PAGADO,Tipo.FACTURA,1,1,1);
        System.out.println("Existe documento: " + existe);
    }
    ...
    private static void testPersonaBOObtenerPorId(ArrayList<Integer> listaId) {
        ...
        for (Integer id : listaId) {
            ...
            System.out.println("idDocumento: " + documento.getIdDocumento() + " " + documento.getEstado().toString()+ " " + documento.getTipo().toString());
        }
    }
    
    private static void testPersonaBOModificar(ArrayList<Integer> listaId) {
        System.out.println("\ntestPersonaBOModificar");
        Integer resultado = documentoBO.modificar(listaId.get(0), 1,Estado.COTIZACION,Tipo.VENTA,1,1,1);
        resultado = documentoBO.modificar(listaId.get(1), 1,Estado.ANULADO,Tipo.COMPRA,1,1,1);
